...
## Variables
These are the config variables available as of CraftOS-PC v2.2.4. *Variables marked `global.json` only are not exported through the `config` API and must be edited manually.*
